...
public class HaykamChunkGenerator extends ChunkGenerator {
			...
			for (int zo = 0; zo < 16; ++zo) {
				...
				for (int y = 0; y < world.getHeight(); ++y) {
					...
					if (y < height) {
						...
						chunk.setBlockState(pos, net.minecraft.block.Blocks.WATER.getDefaultState(), false);
					} else {
						chunk.setBlockState(pos, net.minecraft.block.Blocks.AIR.getDefaultState(), false);
					}
				}
			}
		}
	...
	public int getHeight(int x, int z, Heightmap.Type heightmapType) {
		int height = getHeight(x, z);
		return heightmapType.getBlockPredicate().test(net.minecraft.block.Blocks.WATER.getDefaultState()) ? Math.max(height, this.getSeaLevel()) : height;
	}
			...
			for (int z = 0; z < 16; z++) {
				...
				for (int y = 255; y >= 0; --y) {
					...
					if (y <= random.nextInt(6) - 1) {
						chunk.setBlockState(new BlockPos(x, y, z), net.minecraft.block.Blocks.BEDROCK.getDefaultState(), false);
					} else if (y == 0){
						chunk.setBlockState(new BlockPos(x, y, z), net.minecraft.block.Blocks.BEDROCK.getDefaultState(), false);
					} else {
						...
						if (currentBlock == net.minecraft.block.Blocks.AIR) {
							...
							if (run == -1) {
								if (stoneSampleAtPos <= 0) {
									topState = net.minecraft.block.Blocks.AIR.getDefaultState();
								...
								if ((y < seaLevel) && topState.isAir()) {
									topState = net.minecraft.block.Blocks.WATER.getDefaultState();
								}
	...
	public BlockView getColumnSample(int x, int z) {
		...
		for (int y = 0; y < 256; ++y) {
			if (y < height) {
				...
				column[y] = net.minecraft.block.Blocks.WATER.getDefaultState();
			} else {
				column[y] = net.minecraft.block.Blocks.AIR.getDefaultState();
			}
		}
